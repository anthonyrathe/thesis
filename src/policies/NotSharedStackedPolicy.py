...
class NotSharedStackedPolicy(ActorCriticPolicy):
	"""
	...
	def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch, n_groups=2, group_size=4,n_first_layer_input_features=9,second_layer=True, reuse=False, layers=None, net_arch=None,
		...
		super(NotSharedStackedPolicy, self).__init__(sess, ob_space, ac_space, n_env, n_steps, n_batch, reuse=reuse,
...
def mlp_extractor(observations_1, observations_2, net_arch, act_fun, n_groups, group_size):
	...
	for idx, layer in enumerate(net_arch):
		if isinstance(layer, str):
			...
			if parallel:
				for i in range(len(latents)):
					latents[i] = act_fun(linear(latents[i], "shared_fc{}_{}".format(idx,i), layer_size, init_scale=np.sqrt(2)))
			else:
	...
	for idx, (pi_layer_size, vf_layer_size) in enumerate(zip_longest(policy_only_layers, value_only_layers)):
		if pi_layer_size is not None:
			if isinstance(pi_layer_size, str):
				...
				if policy_parallel:
					for i in range(len(latents_policy)):
						latents_policy[i] = act_fun(linear(latents_policy[i], "pi_fc{}_{}".format(idx,i), pi_layer_size, init_scale=np.sqrt(2)))
				else:
			...
			if isinstance(vf_layer_size, str):
				...
				if value_parallel:
					for i in range(len(latents_value)):
						latents_value[i] = act_fun(linear(latents_value[i], "vf_fc{}_{}".format(idx,i), vf_layer_size, init_scale=np.sqrt(2)))
				else:
