...
# |	--> Cfr. Super Experiment 2, but without peer-group-awareness, i.e.:				  |
# | 			random peer-normalized groups at input   							      |
# +---------------------------------------------------------------------------------------+
super_experiment_name = "super_experiment_4"
...
peer_group_aware = False
n_permutations = 100
permutation_start_index = 0
ultimate_expert = False
expert_name = "no_second_layer_not_peer_group_aware"
pre_training = True
window = 1
reward_type="long_term_p&l_100"
...
if __name__ == '__main__':
	for env_type in [StackedEnvBinary, StackedEnv, StackedEnvDiff]:
		for transaction_cost in [1e-11, 0.0002, 0.005]:
			if len(sys.argv) > 2:
				if [StackedEnvBinary, StackedEnv, StackedEnvDiff].index(env_type) != int(sys.argv[1]) \
