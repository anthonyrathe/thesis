	...
	def __init__(self,tickers,transaction_cost,train_test_split=0.8,realized=False,reward="sharpe",include_cash=True,**kwargs):
		...
		for ticker in tickers:
			...
			for field in ['EV/EBITDA','P/E','P/B']:
				data['{}_v'.format(field)] = (data[field] - data[field].shift(1))
				data['{}_a'.format(field)] = (data['{}_v'.format(field)] - data['{}_v'.format(field)].shift(1))
		...
		self.observation_space = gym.spaces.Box(low=-np.Inf,high=np.Inf,shape=(int(len(self.historical_data.columns)/len(tickers))*2+2,),dtype=np.float32)

		self.action_space = gym.spaces.Box(low=-1.0,high=1.0,shape=(2+int(self.include_cash),),dtype=np.float32)
