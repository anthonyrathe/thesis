...
for peer_group in tickers:
	if len(peer_group) == 1:
		peer_combinations = (peer_group[0],peer_group[0])
	else:
...
if peer_group_aware:
	...
	combinations = list(itertools.permutations(tickers_flattened,2))

print("{} combinations: {}".format(str(len(combinations)),combinations))
start = True
if train:
	count = 0
	for combination in random.sample(combinations,len(peer_combinations)):
		print("{}: {}%".format(combination,count/len(peer_combinations)*100))
		count += 1
